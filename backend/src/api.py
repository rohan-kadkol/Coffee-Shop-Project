...
from database.models import db_drop_and_create_all, setup_db, Drink, db_create_all
...
def patch_drinks(payload, drink_id):
    ...
    try:
        ...
        drink.recipe = json.dumps(body['recipe']) if body.get('recipe', None) else drink.recipe
        drink.update()
        drinks = [drink.long()]
        return jsonify({
            'success': True,
